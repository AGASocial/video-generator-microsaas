...
must_haves:
  truths:
    - "A top-to-bottom runbook exists covering all production verification steps"
    - "Every production env var is listed with its correct name and source"
    - "SQL queries for RLS verification are included verbatim"
    - "Smoke tests cover video proxy, password reset, Stripe checkout, and download"
  artifacts:
    - path: ".planning/phases/03-security-storage-launch/PRE-DEPLOY-RUNBOOK.md"
      provides: "Pre-deploy operator checklist"
      contains: "KLING_WEBHOOK_SKIP_VERIFICATION"
  key_links:
    - from: "PRE-DEPLOY-RUNBOOK.md"
      to: "Vercel dashboard"
      via: "env var checklist"
      pattern: "NEXT_PUBLIC_APP_URL"
    - from: "PRE-DEPLOY-RUNBOOK.md"
      to: "Supabase SQL editor"
      via: "RLS verification SQL"
      pattern: "rowsecurity"
---

<objective>
Write the pre-deploy runbook that an operator works through top-to-bottom before going live.

Purpose: The platform is code-complete after Wave 1 fixes. What remains is a structured verification pass — env vars, RLS, external service setup, and smoke tests — executed in the correct order so nothing is missed before accepting real users.

Output: `.planning/phases/03-security-storage-launch/PRE-DEPLOY-RUNBOOK.md`
</objective>
...
@.planning/phases/03-security-storage-launch/03-01-SUMMARY.md
@.planning/phases/03-security-storage-launch/03-02-SUMMARY.md
</context>

<tasks>

<task type="auto">
  <name>Task 1: Write PRE-DEPLOY-RUNBOOK.md</name>
  <files>.planning/phases/03-security-storage-launch/PRE-DEPLOY-RUNBOOK.md</files>
  <read_first>
    - .planning/phases/03-security-storage-launch/03-CONTEXT.md — canonical env var list and scope
    - .env.dev — current dev env var names; production names match (except values change)
    - scripts/001_create_tables.sql — RLS policies defined here; extract table names for verification SQL
  </read_first>
  <action>
Create `.planning/phases/03-security-storage-launch/PRE-DEPLOY-RUNBOOK.md` with the following exact structure and content. Write the full file — do not truncate.
...
Write this file in full. Do not truncate or summarize any section.
  </action>
  <verify>
    <automated>grep -c "\- \[ \]" /Users/gabrielvega/Projects/agasocial/videoai/video-generator-microsaas/.planning/phases/03-security-storage-launch/PRE-DEPLOY-RUNBOOK.md</automated>
  </verify>
  <acceptance_criteria>
    - File exists at `.planning/phases/03-security-storage-launch/PRE-DEPLOY-RUNBOOK.md`
    - `grep -c "\- \[ \]" PRE-DEPLOY-RUNBOOK.md` outputs a number >= 25 (all checkbox items present)
    - `grep "KLING_WEBHOOK_SKIP_VERIFICATION" PRE-DEPLOY-RUNBOOK.md` returns at least 2 matches (listed both in "must not be set" and final checklist)
    - `grep "NEXT_PUBLIC_APP_URL" PRE-DEPLOY-RUNBOOK.md` returns at least 3 matches (table, verification command, final checklist)
    - `grep "rowsecurity" PRE-DEPLOY-RUNBOOK.md` returns the SQL verification query
    - `grep "deduct_credits_and_create_video" PRE-DEPLOY-RUNBOOK.md` returns at least 2 matches (SQL query and checklist item)
    - `grep "302" PRE-DEPLOY-RUNBOOK.md` returns the smoke test for video proxy
    - `grep "reset-password" PRE-DEPLOY-RUNBOOK.md` returns at least 2 matches (Supabase template section and smoke test)
    - All 7 sections are present: `grep "## Section [1-7]" PRE-DEPLOY-RUNBOOK.md | wc -l` outputs `7`
  </acceptance_criteria>
  <done>PRE-DEPLOY-RUNBOOK.md exists with all 7 sections, all checkbox items, SQL verification queries, and smoke test commands. An operator can work through it top-to-bottom with no prior knowledge.</done>
</task>
...
|----------|-------------|
| Operator → Vercel dashboard | Env vars set by human; runbook ensures correct values |
| Runbook → production deploy | No automated deployment; operator follows checklist |

## STRIDE Threat Register

| Threat ID | Category | Component | Disposition | Mitigation Plan |
|-----------|----------|-----------|-------------|-----------------|
| T-03-03-01 | Information Disclosure | Runbook contains env var names | accept | File is in .planning/ (not committed to public repo); names only, no values |
| T-03-03-02 | Tampering | KLING_WEBHOOK_SKIP_VERIFICATION set in prod | mitigate | Runbook explicitly calls out this var must NOT be set; final checklist verifies absence |
</threat_model>
...
```bash
# Confirm file exists
ls -la .planning/phases/03-security-storage-launch/PRE-DEPLOY-RUNBOOK.md

# Confirm all 7 sections present
grep "## Section" .planning/phases/03-security-storage-launch/PRE-DEPLOY-RUNBOOK.md

# Confirm checkbox count
grep -c "\- \[ \]" .planning/phases/03-security-storage-launch/PRE-DEPLOY-RUNBOOK.md

# Confirm critical warnings present
grep "KLING_WEBHOOK_SKIP_VERIFICATION" .planning/phases/03-security-storage-launch/PRE-DEPLOY-RUNBOOK.md
```
</verification>

<success_criteria>
- `PRE-DEPLOY-RUNBOOK.md` exists and is complete (>= 25 checkboxes, 7 sections)
- All phase requirements are traceable to runbook sections: INFRA-01/02 (env vars + RLS), VID-03 (download smoke test), VID-04 (video proxy smoke test), AUTH-03 (password reset flow), ERR-02 (Stripe checkout error path)
- An operator with no prior context can follow the runbook from Section 1 to Section 7 without needing to consult other documents
</success_criteria>

<output>
After completion, create `.planning/phases/03-security-storage-launch/03-03-SUMMARY.md`
</output>
